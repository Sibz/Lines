...
namespace Sibz.Bezier
{
    public class Bezier
    {
        ...
        {
            var a = Vector2.Lerp(Vectors[0], Vectors[1], t);
            var b = Vector2.Lerp(Vectors[1], Vectors[2], t);

            if (Vectors.Length == 3) return Vector2.Lerp(a, b, t);

            var c  = Vector2.Lerp(Vectors[2], Vectors[3], t);
            var a2 = Vector2.Lerp(a, b, t);
            var b2 = Vector2.Lerp(b, c, t);
        ...
        {
            var a = math.lerp(v0, v1, t);
            var b = math.lerp(v1, v2, t);
            var c = math.lerp(a, b, t);

            return c;

            //return math.lerp(math.lerp(math.lerp(v0, v1, t), b, t), math.lerp(b, math.lerp(v2, v3, t), t), t);
        }
        public static float2 GetVectorOnCurve(float2 v0, float2 v1, float2 v2, float t)
        {
            var a = math.lerp(v0, v1, t);
            var b = math.lerp(v1, v2, t);
            var c = math.lerp(a, b, t);
        ...
        {
            return GetVectorOnCurve(curve.c0, curve.c1, curve.c2, curve.c3, t);
        }
    }
    ...
    public class CircularBezierSpline
    {
        ...
        private Vector2[] GetControlPoints(int index)
        {
            ...
            var previousVector = Vectors[index - 1 < 0 ? Vectors.Length - 1 : index - 1];
            var nextVector     = Vectors[index + 1 >= Vectors.Length ? 0 : index + 1];

            // Calculate vector from previous vector to the next
            var heading = nextVector - previousVector;
                                                 ...
                                                 float                                     tension = 0.66f)
        {
            ...
            float looseness      = (1f - tension) / 2;
            var   previousVector = knots[index - 1 < 0 ? knots.Count() - 1 : index - 1];
            var   nextVector     = knots[index + 1 >= knots.Count() ? 0 : index + 1];
        ...
        public Vector2 GetPointOnSpline(float t)
        {
            ...
            var secondKnotVector = Vectors[secondKnotIndex];

            // Get second controlpoint for this knot
            var firstControlPoint = GetControlPoints(firstKnotIndex)[1];
            // Get first controlpoint for next knot
            var secondControlPoint = GetControlPoints(secondKnotIndex)[0];

            var bezier = new Bezier(Vectors[firstKnotIndex], firstControlPoint, secondControlPoint,
                                    secondKnotVector);
        ...
        public static float3 GetPointOnSpline([ReadOnly] ref NativeHashMap<int, float3> knots, float t,
                                              float                                     tension = 0.66f)
        {
            var firstKnotIndex = (int) math.floor(t * knots.Count());
            var newT           = 0f;
        ...
        {
            var firstKnotIndex = (int) math.floor(t * knots.Length);
            var newT           = 0f;
