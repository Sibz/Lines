...
namespace Sibz.Lines.ECS.Jobs
{
    ...
    {
        [ReadOnly] public ComponentDataFromEntity<Line> Lines;
        [ReadOnly] public ComponentDataFromEntity<LineJoinPoint> JoinPoints;
        [ReadOnly] public NativeArray<NewLineUpdateEvent> EventData;
        ...
        {
            Entity lineEntity = EventData[i].LineEntity;
            if (!Lines.Exists(lineEntity))
            {
                return;
            }

            LineEntities[i] = lineEntity;
            Line line = Lines[lineEntity];
            LineWithJoinData[i] = new LineWithJoinPointData
            {
                Line = line,
                JoinPointA = JoinPoints.Exists(line.JoinPointA) // It should exist, but just in case
                    ? JoinPoints[line.JoinPointA]
                    : throw new InvalidOperationException("JoinPointA didn't exist"),
                JoinPointB = JoinPoints.Exists(line.JoinPointB) // It should exist, but just in case
