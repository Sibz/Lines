...
namespace Sibz.Lines.ECS.Jobs
{
    public struct LineToolUpdateGatherJoinsJob : IJob
    {
        ...
        public void Execute()
        {
            Line line = Em.GetComponentData<Line>(LineEntity);

            AddJoinPoint(line.JoinPointA);
            AddJoinPoint(line.JoinPointB);

            if (!Em.Exists(JoinToEntity))
            {
                return;
            }
        ...
        {
            LineJoinPoint jp = Em.GetComponentData<LineJoinPoint>(joinPointEntity);
            JoinEntities.Add(joinPointEntity);
            JoinPoints.Add(jp);
            if (!Em.Exists(jp.JoinToPointEntity))
            {
                return;
            }
