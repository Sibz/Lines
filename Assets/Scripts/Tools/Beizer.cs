...
public class Bezier
{
    ...
    {
        if (!(vectors.Length == 4 || vectors.Length == 3)) throw new ArgumentException("Bezier constructor requires array with 3 or 4 vectors");
        Vectors = vectors;
    }
...
public class CircularBezierSpline
{
    ...
    private static float3x2 GetControlPoints([ReadOnly] ref NativeHashMap<int, float3> knots, int index, float tension = 0.66f)
    {
    ...
    public Vector2 GetPointOnSpline(float t)
    {
        ...
        int firstKnotIndex = (int)Math.Floor(t * (Vectors.Length)); // 0.1 * 10 = 1 ; 0.5 * 10 = 5 ; 0.15 * 0 = 1.5 (1) ; 1 * 10
    ...
    public static float3 GetPointOnSpline([ReadOnly] ref NativeHashMap<int, float3> knots, float t, float tension = 0.66f)
    {
