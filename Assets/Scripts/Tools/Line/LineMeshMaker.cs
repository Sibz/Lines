...
namespace Sibz.Lines
{
    public static class LineMeshMaker
    {
        ...
        {
            var result = new Mesh();
            if (knots.Length == 0) return result;

            var   vertices  = new Vector3[knots.Length * 2];
            var   triangles = new int[(knots.Length - 1) * 6];
            var   normals   = new Vector3[knots.Length * 2];
            var   uvs       = new Vector2[knots.Length * 2];
            var   up        = new float3(0, 1, 0);
            float halfWidth = width / 2f;
            for (var i = 0; i < knots.Length; i++)
            {
                var knot = knots[i];
                var direction = i == 0 ? -originForward :
        ...
        {
            if (linePoints.Length < 2) throw new ArgumentException("Should have 2 or more vectors", nameof(linePoints));

            if (linePoints.Length == 2 && sections == 1) return BuildQuad(linePoints[0], linePoints[1], width);
        ...
        private static Mesh BuildQuad(Vector3 point1, Vector3 point2, float width)
        {
            ...
            var   up    = Vector3.up;
            var   cross = Vector3.Cross(up, point2 - point1).normalized;
            float zMod  = cross.z * halfWidth;
            float xMod  = cross.x * halfWidth;
