    ...
    public class LineToolOld
    {
        ...
        public LineToolOld(GameObject cursor, GameObject linePrefab, LineToolBehaviour tool)
        {
            this.linePrefab = linePrefab == null
                                  ? throw new ArgumentNullException(nameof(linePrefab))
                                  : linePrefab;
            this.cursor  = cursor == null ? throw new ArgumentNullException(nameof(cursor)) : cursor;
            this.tool    = tool;
        ...
        {
            var tx       = cursor.transform;
            var rotation = tx.rotation;
            var position = tx.position;

            originSnappedToNode = snapNotifier.SnappedTo;

            CurrentLine = UnityEngine.Object.Instantiate(linePrefab.gameObject, position, rotation)
            ...
            CurrentLine.OriginNode.transform.rotation =
                originSnappedToNode
                    ? Quaternion.LookRotation(
                                              CurrentLine.transform.InverseTransformDirection(-originSnappedToNode
                                                                                              .transform.forward))
                    : rotation;
        ...
        {
            var    originTx = CurrentLine.OriginNode.transform;
            var    endTx    = CurrentLine.EndNode.transform;
            ...
            else
            {
                ...
                {
                    if (!originSnappedToNode) originTx.LookAt(endTx.position);

                    if (!endSnappedToNode) endTx.LookAt(originTx.position);
                }
            ...
            float3 GetOrigin()
            {
                return GetControlPoint(forwards.c0, lineEndA, distances.x, scales.x, ratio1);
            }

            float3 GetEnd()
            {
                return GetControlPoint(forwards.c1, lineEndB, distances.y, scales.y, ratio2);
            }
            ...
            {
                return LineHelpers.AngleDegrees(a, b);
            }
        ...
        private void UpdateKnots()
        {
            ...
            switch (localMode)
            {
                ...
                {
                    var knots = GetPartCurveKnots(curve1);
                ...
                case LocalToolMode.StraightOriginAndEndCurves:
                    var knots1 = GetPartCurveKnots(curve1);
                    var knots2 = GetPartCurveKnots(curve2, true);
        ...
        private float3[] GetPartCurveKnots(
            ...
            bool     invert = false)
        {
            var originPos    = bezier.c0;
            var controlPoint = bezier.c1;
            var endPos       = bezier.c2;

            if (controlPoint.Equals(originPos)) return new float3[0];

            float lineDistance = math.distance(originPos, controlPoint) + math.distance(controlPoint, endPos);

            var numberOfKnots = (int) math.ceil(lineDistance / tool.KnotSpacing);
            var knots         = new float3[numberOfKnots];
        ...
        private void AdjustLinePosition()
        {
            ...
            var originTx       = CurrentLine.OriginNode.transform;
            var endTx          = CurrentLine.EndNode.transform;
            var originPos      = originTx.position;
            var endPos         = endTx.position;
            var originRotation = originTx.rotation;
            var endRotation    = endTx.rotation;

            var midWayPoint = Vector3.Lerp(
            ...
            if (originSnappedToNode) originTx.rotation = originRotation;

            if (endSnappedToNode) endTx.rotation = endRotation;
        }
        ...
        private void MoveEndNode()
        {
            ...
            if (endSnappedToNode)
            {
                var endSnappedToTx = endSnappedToNode.transform;
        ...
        public void Cancel()
        {
            if (CurrentLine) UnityEngine.Object.Destroy(CurrentLine.gameObject);
        }
